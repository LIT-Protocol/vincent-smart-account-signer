...
export async function sendPermittedUserOperation({
  ...
  const aaveUserOpHash = await permissionKernelClient.sendUserOperation(signedUserOp);
  console.log('UserOp hash:', aaveUserOpHash);

  const aaveUserOpReceipt = await permissionKernelClient.waitForUserOperationReceipt({
    hash: aaveUserOpHash,
  });
